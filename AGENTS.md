...
Owns .envrc.dc — the scalar-config/secret SoT for trl-infra (AWS, Docker, Helm, Infisical creds). direnv lib + shell hook + CLI; credential layering dc: → override: → auto: → default:. Note: `dc --auto` does not persist generated values — use `dc config set --yes`.
